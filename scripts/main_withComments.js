...
//        });
    ...
    function updateColorIntensities(map, countries, allObjs){
        ...
        var legendArray = [200, 400, 600, 800, 1000, 1200]; /// HARD CODED.. IT'S OKAY AS LONG AS WE DON'T REDOWNLOAD THE DATA
        if(counts.length != 0){
...
//                            d3.select('.comparisonPanel').selectAll('div.comp').filter(function(d){ return $(this).attr('id') == i; }).select('svg').selectAll('rect.fromWhere').enter().append('rect')
//                                .attr('x', 5)
//                                .attr('y', 10)
//                                .attr('width',5)
//                                .attr('height',5)
//                                .style({'fill': '#999'});
        ...
        function drawScatter(inComparison){
            ...
            if(teamIds.length > 0){
                ...
                // DRAW X AXIS TICKS
//            funcPrepData += 'console.log(logAxis(memberCountsStorage)(1200));';
                ...
                // FOR EACH TEAM, GET THE TEAM NAME, SHORT DESCRIPTION, NUMBER OF MEMBERS, AMOUNT LENT and...
                // FILTER BASED ON THE COUNTRY SELECTED
//            funcPrepData == 'var filteredTeamInfo = []; $(teamInfo).filter(function(i){ return countryFilter == this.teamId }).each(function(){ filteredTeaminfo.push(this); });';
                ...
                // var isBooked = 0; $(".comp").each(function(){ if($(this).hasClass("occupied")){ isBooked +=1; } }); console.log(isBooked);
...
//            d3.selectAll(".comp").selectAll("text").on("mouseover", function(d,i){ console.log('asd'); });
        }
...
//            d3.select('#countryNames').selectAll('path')
//                .data(filteredObjs)
//                .enter()
//                .append('path')
//                .attr('class', function(d){ return 'subunit-label-'+ d.id; })
//                .attr('d', function(d){ return 'M ' + path.centroid(d)[0] + ' ' + parseInt(path.centroid(d)[1]+10) + ' l 3 17 l 15 0 l 0 20 l -36 0 l 0 -20 l 15 0 l 3 -17';})
//                .style({'fill':'rgba(0,0,0,1)', 'stroke': '#000000', 'stroke-width': 1})//.style('display','none')
//
//            d3.select('#countryNames').selectAll('text')
//                .data(filteredObjs)
//                .enter()
//                .append('text')
//                .attr('class', function(d){ return 'country-tooltip country-tooltip-'+ d.id; })
//                .attr('x', function(d){ return path.centroid(d)[0]-12;})//.attr('transform', function(d){ return "translate("+path.centroid(d)+")";})
//                .attr('y', function(d){ return path.centroid(d)[1]+41;})
//                .text(function(d){ return d.teams;})
//                .style({'fill': '#ffffff', 'stroke': '#ffffff', 'stroke-width': 1, 'cursor': 'pointer'})//.style('display','none')
            ...
            $('.dropdown-menu li').click(function(){
                ...
                $.getJSON('data/'+$(this).find('a').attr('id')+'.json', function(data){
                    ...
                    for(var i in source.loans){
                        ...
                        if(counterTo100 > 100){
//                        console.log(counterTo100, loansId);
                            break;
                        }
                    }
            ...
            // GET 500 LOANS OF THE SELECTED SECTOR
            // FOR EACH LOAN, SAVE THE TEAM ID UNTIL IT REACHES 500
            // FOR EACH TEAM, SAVE THE AMOUNT OF LOANS (** THIS NEEDS CLARIFICATION) AND LOCATION

            // SNAPSHOT AS OF 12/12/13 14:36


//            while(teams500.id.length < 10){
//                loans = snapshotAgriculture.loans;
//                lender_counter = snapshotAgriculture.loans[loan].lender_count;
//                if(lender_counter != 0){
////                    teamsInThisLoanURL = 'http://api.kivaws.org/v1/loans/'+loanId+'/teams.json';
//                    $.get('/',function(data2){
//                        console.log(1);
//                        teams500.id.push(1);
//                    });
////                    $.get(teamsInThisLoanURL, function(data2){
////                        for(var team in data2.teams){
////                            teams500.id.push(data2.teams[team].id);
////                        }
////                    });
//                }
//
